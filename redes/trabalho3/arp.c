...
int main (int argc, char **argv) {
	...
	printf ("Index for interface %s is %i\n", interface, device.sll_ifindex);// Set destination MAC address: broadcast address
	...
	arphdr.htype = htons (1); // Protocol type (16 bits): 2048 for IP
	...
	ether_frame[13] = ETH_P_ARP % 256;// Next is ethernet frame data (ARP header).
	...
	allocate_strmem (int len) {
		void *tmp;
		if (len <= 0) {
			fprintf (stderr, "ERROR: Cannot allocate memory because len = %i inallocate_strmem().\n", len);
			exit (EXIT_FAILURE);
		}
